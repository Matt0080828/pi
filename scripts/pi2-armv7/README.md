...
| `pi2-pi-agent.sh` | the PC | `probe` (reachability + current state), `install` (fetches the Node tarball on the PC, scp's it over, runs the installer, keeps a log), `verify` (judges a log against the acceptance list), `all`. Host via `HOST=pi2@192.168.50.90`. |
