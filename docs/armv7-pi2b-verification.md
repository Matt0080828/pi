...
=== 4) 量測（給日後比對用）
    ...
    node RSS 基準: 40 MiB
    磁碟：Node 187M｜pi 套件 156M（注意：/home/pi2/.local 可能與其他工具共用，勿整包刪）

=== 5) 接下來（可選，需要你的 API key 或本機端點）
    export PATH="/home/pi2/opt/node22/bin:/home/pi2/.local/bin:$PATH"     # 加進 ~/.bashrc 才會持久
    pi --version
